...
# 定义一个生成数据集的方法，并放入离群数据
def gener_data():
    '''
    随机生成包含500哥样本的数据集，其中有100个离群数据
    :return: 一个包含离群数据的数据集
    '''
    size = (500,1)
    X = np.random.random(size) * 20
    k = 10 * np.random.standard_normal()
    Y = X * k + 10 * np.random.standard_normal(size)
    X = X + 10 * np.random.standard_normal(size)
    data = np.hstack((X, Y))
    # 加入离群数据
    index = np.random.randint(500, size=100)
    data[index] = [np.random.random() * 30, np.random.random() * 30]
...
def ransac(data, model, n, k, t, d):
    ...
    需要的参数：数据集，取随机数的个数，循环的次数，假设的模型，离群数据的阈值，内群数据的最低标准。
    :return:
    '''
    iterations = 0
    bestfit = None

...
if __name__ == '__main__':
    ...
    print('image', img)
    if pic_path[-4:] == '.png':  # .png图片在这里的存储格式时0到1的浮点数，所以要扩展到255再计算。
        img = img * 255
    ...
    dim = 5 # 高斯核尺寸
    Gaussian_filter = np.zeros([dim,dim]) # 存储高斯核，这里是数组不是列表了。
    ...
    dx,dy = img.shape
    img_new = np.zeros(img.shape) # 存储平滑后的图像，zeros函数得到的时浮点型数据。
    tmp = dim // 2
    ...
    以下两个是滤波用的sobel矩阵（检测图像中的水平，垂直和边角边缘)
    ...
    img_pad = np.pad(img_new,((1,1),(1,1)),'constant') # 边缘填补，根据上面的矩阵结构所以写1
    for i in range(dx):
        for j in range(dy):
    ...
    for i in range(1,dx -1):
        for j in range(1,dy -1):
            ...
            if angel[i,j] <= -1: # 使用线性插值法判断抑制与否？
                num_1 = (temp[0,1] - temp[0,0]) / angel[i,j] + temp[0,1]
                num_2 = (temp[2,1] - temp[2,2]) / angel[i,j] + temp[2,1]
                if not(img_tidu[i,j] > num_1 and img_tidu[i,j] > num_2):
                    flag = False
            elif angel[i,j] >= 1:
                num_1 = (temp[0,2] - temp[0,1])/ angel[i,j] + temp[0,1]
                num_2 = (temp[2,0] - temp[2,1])/ angel[i,j] + temp[2,1]
                if not (img_tidu[i,j] > num_1 and img_tidu[i,j] > num_2):
                    flag = False
            elif angel[i,j] > 0:
                num_1 = (temp[0,2] - temp[1,2])/ angel[i,j] + temp[1,2]
                num_2 = (temp[2,0] - temp[1,0])/ angel[i,j] + temp[1,0]
                if not (img_tidu[i,j] > num_1 and img_tidu[i,j] > num_2):
                    flag = False
            elif angel[i,j] >= 1:
                num_1 = (temp[1,0] - temp[0,0])/ angel[i,j] + temp[1,0]
                num_2 = (temp[1,2] - temp[2,2])/ angel[i,j] + temp[1,2]
                if not (img_tidu[i,j] > num_1 and img_tidu[i,j] > num_2):
                    flag = False
            if flag:

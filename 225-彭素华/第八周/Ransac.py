...
class LinearLeastSquareModel:
    ...
    def fit(self, data):
        ...
        '''
         ...
         sl.lstsq 是 SciPy 提供的求解线性最小二乘问题的函数。它的作用是求解如下形式的线性方程组的最小二乘解:
         Ax≈B
         ...
         该函数返回以下四个值：
             ...
             x：表示最小二乘解
             resids：残差和。 残差和 resids 是一个重要的指标，用于评估最小二乘解的质量。它表示了拟合误差的大小，即模型预测值与实际值之间的差异。如果残差和很小，说明模型的拟合效果较好；如果残差和很大，说明模型的拟合效果较差。
             rank：矩阵 A 的秩。
             s：A 的奇异值。
        '''
...
def test():
    ...
    n_inputs = 1
    n_outputs = 1
    A_exact = 20 * np.random.random((n_samples,n_inputs))  # np.random.random() 生成一个随机浮点数
    ...
    linear_fit, resids, rank, s = sp.linalg.lstsq(all_data[:, input_columns],
                                                  all_data[:, output_columns])  # linalg函数，一个库
    if 1:
        import pylab

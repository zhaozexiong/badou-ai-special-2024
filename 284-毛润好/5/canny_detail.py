...
if __name__ == '__main__':
    ...
    if pic_path[-4:] == '.png':  # .png图片在这里的存储格式是0到1的浮点数，所以要扩展到255再计算
        img = img * 255  # 还是浮点数类型
    img = img.mean(axis=-1)  # 取均值的方法进行灰度化（按最后一个轴，颜色通道轴）
    ...
    # 2、求梯度。并检测水平、垂直、对角边缘
    sobel_kernel_x = np.array([[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]])  # 水平sobel
    sobel_kernel_y = np.array([[1, 2, 1], [0, 0, 0], [-1, -2, -1]])  # 垂直sobel

    img_tidu_x = np.zeros(img_new.shape)  # 初始化3个矩阵
    img_tidu_y = np.zeros([dx, dy])  # 存储梯度图像
    img_tidu = np.zeros(img_new.shape)  #

    img_pad = np.pad(img_new, ((1, 1), (1, 1)), 'constant')  # 边缘填补，sobel矩阵是3*3的所以写1
    for i in range(dx):
        for j in range(dy):
    ...
    img_tidu_x[img_tidu_x == 0] = 0.00000001  # img_tidu_x要作除数 避免除零错误
    ...
    for i in range(1, dx - 1):
        for j in range(1, dy - 1):
            ...
            if angle[i, j] <= -1:  # 使用线性插值法判断抑制与否
                ...
                if not (img_tidu[i, j] > num_1 and img_tidu[i, j] > num_2):
                    flag = False
            if flag:  # flag为True，就是用原来img_tidu的值；false，用img_yizhi的值初始化时默认为0
    ...
    while not len(zhan) == 0:
        ...
        a = img_yizhi[temp_1 - 1:temp_1 + 2, temp_2 - 1:temp_2 + 2]  # 切片不包含末尾

# 1.对图像灰度化
# 2.对图像进行高斯滤波
#     根据待滤波的像素点及其附近领域点的灰度值按照一定的参数规则进行加权平均。这样
#     可以有效虑去理想图像中叠加的高频噪声
# 3.检测图像中的水平、垂直和对角边缘（如Prewitt、sobel算子等）
# 4.对梯度幅值进行非极大值抑制，可以剔除一大部分非边缘的点
# 5.用双阈值算法检测和连接边缘（完成非极大值抑制后，可能包含很多由噪声及其他原因造成的假边缘）
...
if __name__ == '__main__':
    pic_path = '../0327/lenna.png'
    img = plt.imread(pic_path)  # 读取图片
    print('source', img)
    # plt.imshow(img, cmap='gray')
    if pic_path[-4:] == '.png':  # 如果图片后缀是.png，其存储格式是0到1，所以需要扩展到255再计算
        img = img * 255
    print('kuozhanhou', img)
    img = img.mean(axis=-1)  # 用均值的方法灰度化
    # 高斯滤波
    sigma = 0.5  # 高斯平滑时的高斯核参数，标准差，可调
    dim = 5  # 高斯核尺寸，卷积核5*5，还有3*3等
    # 高斯卷积核
    ...
    tmp = dim // 2
    img_pad = np.pad(img, ((tmp, tmp), (tmp, tmp)), 'constant')  # 填补边缘
    for i in range(dx):
        for j in range(dy):

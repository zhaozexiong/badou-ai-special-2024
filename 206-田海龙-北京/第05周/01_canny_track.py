...
def CannyThreshold(lowThreshold):
    ...
    detected_edges = cv2.Canny(
        gray, lowThreshold, maxThreshold, apertureSize=kernel_size
    )  # 边缘检测
    ...
    # dst = cv2.bitwise_and(img, img,mask=gray)
    # dst = cv2.bitwise_and(img, gray)
    # dst = detected_edges
...
# 拖拉工具条，要和窗口名称对应上
cv2.createTrackbar(
    "Min threshold", "canny result", lowThreshold, max_lowThreshold, CannyThreshold
)
# 可以设置多个调节杆
cv2.createTrackbar(
    "max threshold", "canny result", maxThreshold, 255, CannyThreshold
)

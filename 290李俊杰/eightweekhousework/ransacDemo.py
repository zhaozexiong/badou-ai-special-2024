...
while current_iteration < max_iteration:
    # 随机选取可能是的内群点
    maybe_inds, test_inds = random_partition(n, all_data.shape[0])
    # print('test_inds = ', test_inds)
    # 获取size(maybe_idxs)行数据(Xi,Yi)
    maybe_liners = all_data[maybe_inds, :]
    # 若干行(Xi,Yi)数据点
    test_points = all_data[test_inds]
    # 拟合模型
    ...
    if (len(also_inliers) > d):
        ...
        if xingerrs < besterrs:
            ...
            dict_idxs={'inliers': best_inlier_idxs}
    current_iteration += 1

# huatu

if dict_idxs['inliers'] is not None:
    ...
    if 1:
        ...
        pylab.plot(x_noise[dict_idxs['inliers'], 0], y_noise[dict_idxs['inliers'], 0], 'bx',
                       label="RANSAC data")
    else:
    ...
    pylab.plot(A_col0_sorted[:, 0],
               np.dot(A_col0_sorted, bestfit)[:, 0],
               label='RANSAC fit')
    pylab.plot(A_col0_sorted[:, 0],

...
for i in range(dim):
	for j in range(dim):
		gaussian_filter[i,j] = n1*math.exp(n2*(tmp[j]**2))
...
for i in range(1,dx-1):
	for j in range(1,dy-1):
		...
		if angle[i,j] <= -1:
			...
			num_1 = (temp[0,2] - temp[1,2]) * angle[i,j] + temp[0,1]
			num_2 = (temp[2,0] - temp[1,0]) * angle[i,j] + temp[1,2]
			if not(img_tidu[i,j] > num_1 and img_tidu[i,j] > num_2):
				flag = False
		elif angle[i,j] > 0:
...
#双阈值检测
lower_boundary = img_tidu.mean()*0.6
...
while not len(zhan) == 0:
	...
	if (a[0,0] < high_boundary) and (a[0,0] > lower_boundary):
		img_yizhi[temp_1-1,temp2-1] = 255
	...
	if (a[1,1] < high_boundary) and (a[1,1] > lower_boundary):
		img_yizhi[temp_1,temp_2] = 255
		zhan.append([temp_1,temp_2])

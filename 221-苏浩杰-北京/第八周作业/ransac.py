...
def ransac(data, model, n, k, t, d, debug=False, return_all=False):
    ...
    for i in range(k):
        ...
        maybe_model = model.fit(maybe_data)
        test_data = data[test_idxs]
        test_error = model.get_error(test_data, maybe_model)
        print('test_err = ', test_error <t)
        also_idxs = test_idxs[test_error < t]
        print ('also_idxs = ', also_idxs)
        also_inliers = data[also_idxs,:]
        if len(also_inliers) > d:
            betterdata=np.concatenate((maybe_data, also_inliers))
...
def test():
    ...
    if 1:
        ...
        if 1:
            ...
            plt(A_noisy[non_outlier_idxs, 0], B_noisy[non_outlier_idxs, 0], 'k.', label='noisy data')
            plt(A_noisy[outlier_idxs, 0], B_noisy[outlier_idxs, 0], 'r.', label='outlier data')

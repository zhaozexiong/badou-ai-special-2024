...
if __name__ == '__main__':
    ...
    Gaussian_filter = np.zeros([dim,dim])   #生成一个尺寸为dim的二维数组
    tmp = [i-dim//2 for i in range(dim)]    #列表表达式，结果为[-2, -1, 0, 1, 2]
    ...
    dx, dy = img.shape
    img_new = np.zeros(img.shape)   #存储平滑后的图像，zeros函数得到的是浮点型数据
    tmp = dim//2
    ...
    plt.figure(1)
    plt.imshow(img_new.astype(np.uint8),cmap='gray')    #此时的img_new是255的浮点型数据，需强制转换，gray灰阶
    plt.axis('off')

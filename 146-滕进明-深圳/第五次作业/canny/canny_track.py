...
def CannyThreshold(lowThreshold):
    ...
    # 原始颜色添加到检测的边缘上
    # 按位“与”操作。对于每个像素，将两幅图像相应位置的像素值分别进行按位“与”运算，输出的结果图像的对应像素值即为这两幅输入图像对应像素值的按位与结果
    # src1 和 src2 分别表示要进行按位“与”操作的两幅输入图像
    # msck 为可选参数，如果指定了掩膜，则只对掩膜对应位置的像素进行按位“与”操作。函数的返回值表示按位“与”运算的结果。
    dst = cv2.bitwise_not(detected_edges)
    cv2.imshow("Canny Edges", dst)

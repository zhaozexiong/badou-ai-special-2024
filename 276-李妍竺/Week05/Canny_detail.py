...
if __name__ == '__main__':
    ...
    # 2、求梯度。sobel矩阵（检测图像中的水平、垂直和对角边缘）
    ...
    img_tidu_x[img_tidu_x == 0] = 0.00000001  #分母不为0
    angle = img_tidu_y / img_tidu_x  #tan值
    plt.figure(2)
    ...
    lower_boundary = img_tidu.mean() * 0.4
    high_boundary = lower_boundary * 2  # 自由设置
    zhan = []
    for i in range(1, img_yizhi.shape[0] - 1):  # 外圈不考虑
        for j in range(1, img_yizhi.shape[1] - 1):
            if img_yizhi[i, j] >= high_boundary:  # 强边缘
                img_yizhi[i, j] = 255
                zhan.append([i, j])
            elif img_yizhi[i, j] <= lower_boundary:  # 非边缘，舍
                img_yizhi[i, j] = 0

    while not len(zhan) == 0:
        temp_1, temp_2 = zhan.pop()  # 出栈。用栈内的强边缘去检测四周，相当于那个点旁边有强边缘
